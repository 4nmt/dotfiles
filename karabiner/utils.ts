          ...
          manipulators: [
            {
              ...
              // Only trigger this command if there are no sublayer being trigger
              // Eg: Hyper + w + j > Hyper + j
              conditions: allSubLayerVariables
                .filter(
                  (subLayerVariable) =>
                    subLayerVariable !==
                    generateSubLayerVariableName(key as KeyCode)
                )
                .map((subLayerVariableName) => ({
                  type: "variable_if",
                  name: subLayerVariableName,
                  value: 0,
                })),
